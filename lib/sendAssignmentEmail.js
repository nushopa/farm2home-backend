...
async function sendOrderAssignedEmail(
  ...
  try {
    await sendEmail(
      ...
      subject,
      "order-assigned" // -> templates/html/order-assigned.html
    );
    ...
    throw error; // re-thrown so callers (and Promise.all) can detect the failure instead of it vanishing silently
  }
}
